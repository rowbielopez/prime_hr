...
import { ConfirmDialog, StatusBadge } from "@/components/foundation";
  ...
  return (
    <div className="space-y-6">
      <Card>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                No. {detail.employee.employeeNo} · {detail.employee.campusName}
                {detail.employee.officeName
                  ? ` / ${detail.employee.officeName}`
                  : ""}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link
                href={`/service-records/${detail.employee.id}/print`}
                target="_blank"
                className={cn(
        ...
        <CardContent className="grid gap-3 pt-4 text-sm md:grid-cols-2 lg:grid-cols-4">
          <Info
            ...
            value={detail.employee.positionTitle}
          />
          <Info
            label="Employment status"
            value={detail.employee.employmentStatus.replace("_", " ")}
          />
          <Info
            label="Employment type"
            value={detail.employee.employmentType}
          />
          <Info
            label="Date hired"
            value={formatDate(detail.employee.dateHired)}
          />
        </CardContent>
      </Card>
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_22rem]">
            ...
            {detail.currentEntry ? (
              <div className="grid gap-3 text-sm md:grid-cols-2">
                <Info
                  ...
                  value={detail.currentEntry.positionTitle}
                />
                <Info
                  label="Appointment status"
                  value={detail.currentEntry.appointmentStatus}
                />
                <Info
                  label="Station / Place"
                  value={detail.currentEntry.stationPlace}
                />
                <Info
                  label="Period"
                  value={`${formatDate(detail.currentEntry.dateFrom)} to Present`}
                />
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">
                No current service record has been marked yet.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="border-b">
            <CardTitle className="text-base">Record Quality Checks</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 pt-4">
            {detail.warnings.length === 0 ? (
              <div className="flex items-center gap-2">
                <StatusBadge tone="active" label="Ready" />
      ...
      <Card>
        <CardHeader className="border-b">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">Service Record Timeline</CardTitle>
            <span className="text-xs text-muted-foreground">
              {sortedEntries.length}{" "}
              {sortedEntries.length === 1 ? "entry" : "entries"} · newest first
            </span>
          </div>
        </CardHeader>
        <CardContent className="pt-5">
          {sortedEntries.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No service record entries found for this employee.
            </p>
          ) : (
            <div>
              {sortedEntries.map((entry, idx) => (
                <div key={entry.id} className="flex gap-4">
                  ...
                  <div
                    className={cn(
                      "min-w-0 flex-1",
                      idx < sortedEntries.length - 1 ? "pb-6" : "pb-0",
                    )}
                  >
                    {/* Date range + badge */}
                    <div className="mb-1.5 flex flex-wrap items-center gap-2">
                      <span className="text-xs font-semibold">
                        ...
                        {entry.isCurrent ? "Present" : formatDate(entry.dateTo)}
                      </span>
                      <StatusBadge
                        tone={
                          entry.archivedAt
                    ...
                    >
                        ...
                        {canEdit && !entry.archivedAt ? (
                          <div className="flex shrink-0 flex-wrap gap-2">
                            ...
                            <ConfirmDialog
                              ...
                              description="This keeps the audit trail and removes the entry from active records. This action cannot be undone."
                              confirmLabel="Archive"
                              variant="destructive"
                              onConfirm={() => archiveEntry(entry)}
                              isPending={isPending}
                            />
                          </div>
                        ) : null}
                      ...
                      <div className="mt-3 grid gap-2 text-sm md:grid-cols-3">
                        ...
                        <Info
                          label="Separation"
                          value={
                            entry.separationDate
                              ? `${formatDate(entry.separationDate)} · ${value(entry.separationCause)}`
                              : "—"
                          }
                        />
