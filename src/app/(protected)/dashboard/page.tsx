...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-6 lg:space-y-7">
      ...
      <section className="relative overflow-hidden rounded-2xl border premium-border bg-surface-panel p-5 shadow-premium-md md:p-6 lg:p-7">
        <div
          className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/40 to-transparent"
          aria-hidden
        />
        <div
          className="absolute -right-20 -top-24 size-64 rounded-full bg-primary/8 blur-3xl"
          aria-hidden
        />
        <div
          className="absolute -bottom-32 left-1/4 size-72 rounded-full bg-brand-gold/8 blur-3xl"
          aria-hidden
        />
        <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_24rem] lg:items-end">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 max-w-3xl text-2xl font-semibold leading-tight text-foreground md:text-4xl">
              A calmer operating cockpit for people, compliance, and campus HR
              decisions.
            </h2>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
              Triage work, monitor PRIME-HRM readiness, and move into the right
              workflow without scanning a long admin menu.
            </p>
          </div>
          <div className="rounded-2xl border premium-border bg-surface-glass/80 p-3 shadow-glass backdrop-blur-xl">
            ...
            <div className="mt-3 rounded-xl border premium-border bg-surface-panel/80 p-3">
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(22rem,0.75fr)]">
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="apple-panel rounded-2xl border premium-border p-5 shadow-premium-sm">
              ...
              {workload.map((item) => {
                ...
                return (
                  <Link
                    key={item.label}
                    href={item.href}
                    className="group flex items-center justify-between gap-3 rounded-xl border premium-border bg-surface-panel/72 p-3 transition-all hover:bg-surface-raised hover:shadow-premium-sm"
                  >
          ...
          <div className="apple-panel rounded-2xl border premium-border p-5 shadow-premium-sm">
            ...
            <div className="mt-4 rounded-xl border premium-border bg-surface-inset/60 p-3 text-xs leading-5 text-muted-foreground">
              {riskTone === "warning"
        ...
        <aside className="apple-panel rounded-2xl border premium-border p-5 shadow-premium-sm">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-sm font-semibold text-foreground">
                Quick Moves
            ...
            {quickActions.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className="group flex items-center justify-between gap-3 rounded-xl border border-transparent p-2.5 transition-all hover:border-border hover:bg-surface-raised"
                >
                  <span className="flex min-w-0 items-center gap-3">
                    <span
                      className={cn(
      ...
      <section className="apple-panel rounded-2xl border premium-border p-5 shadow-premium-sm">
...
function SummaryTile({
  ...
  return (
    <div className="rounded-xl bg-surface-panel px-3 py-3 shadow-premium-sm">
...
function ReadinessMetric({
  ...
  return (
    <div className="rounded-xl border premium-border bg-surface-panel/70 p-3">
